...
beforeAll(async () => {
  ...
  await admin.query(`insert into public.audio_assets (id, learner_id, organisation_id, storage_key, storage_object_hash, sha256, retention_until) values ($1, $2, $3, $4, $5, $6, now() + interval '30 days')`, [ids.audio, ids.learner, ids.organisation, `private-audio/${ids.audio}.webm`, audioHash, audioHash]);
  await admin.query(`insert into public.derived_data_assets (id, learner_id, organisation_id, source_audio_asset_id, asset_kind, storage_key, storage_object_hash, retention_until) values ($1, $2, $3, $4, 'ALIGNMENT', $5, $6, now() + interval '30 days')`, [ids.derived, ids.learner, ids.organisation, ids.audio, `private-derived/${ids.derived}.json`, derivedHash]);
}, 30_000);
...
describe("Priority 1 deletion verification", () => {
  test("records verified deletion receipts and blocks the lifecycle inventory from further processing", async () => {
    ...
    const [request, audio, derived, receipts, audit] = await Promise.all([
      ...
      admin.query("select storage_key, deleted_at, deletion_request_id from public.audio_assets where id = $1", [ids.audio]),
      admin.query("select storage_key, deleted_at, deletion_request_id from public.derived_data_assets where id = $1", [ids.derived]),
