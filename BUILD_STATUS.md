...
| Teacher session tracking and child preferences | Complete | Protected synthetic session history shows safe labels, passage titles, completion/review status and no child tokens; child reader adds local line-spacing and distraction-free focus controls. |
| Reading journey usability | Complete | Teacher launch checklist rechecks selected-learner consent through the protected service, history includes an actionable Review ready filter, and the child canvas provides local sentence-part navigation and a clearly non-scoring visual place marker. |
| Reading journey follow-through | Complete, mock-only | Teacher navigation shows a protected review-ready count; the child has local-only save/return place controls; authorised adult reviews can be printed using an identifier-minimised, synthetic-only layout. |
| Teacher launch readiness | Complete | Session demo uses a protected staff-and-tenant-scoped aggregate consent query rather than guardian-only consent management, resolving teacher launch checks without granting guardian powers. |
| Reading journey export and acknowledgement | Complete, mock-only | Adults can download an identifier-minimised synthetic review PDF; teachers can append a personal alert acknowledgement that dismisses only their own review-ready count; children can clear only their browser-local bookmark. |
...
The current verified baseline is clean TypeScript checking, **81 Vitest tests**, **11 Playwright tests**, and assertion-backed browser routes for learner safety, content workflow, session demo, child reader, and teacher review hand-off. The test suite includes live two-organisation RLS checks, row-normalization tests, contract-boundary log redaction checks, guardian lifecycle/RLS checks, withdrawal/deletion trigger checks, key-revocation receipt verification, content-review RLS tests, approved-passage selection tests, consent-gate contracts, teacher-versus-guardian launch-readiness role-boundary checks, mock-upload metadata validation, synthetic child-token constraints, adult/child data-boundary tests, child-completion hand-off coverage, teacher checklist/filter coverage, child bookmark/clear flow coverage, teacher acknowledgement flow coverage, printable-review control coverage, browser PDF download coverage, and PDF structure verification. The direct preview endpoint is unauthenticated and therefore displays its protected-route loading state; authenticated browser behaviour is the assertion-backed E2E baseline.
