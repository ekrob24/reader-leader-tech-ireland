...
export default function ChildReader() {
  ...
  return <ChildShell><div className={`mx-auto flex max-w-3xl flex-col gap-6 px-4 py-7 sm:px-6 sm:py-10 ${focusMode ? "max-w-4xl py-10 sm:py-16" : ""}`}>
    ...
    {view.state !== "READY_TO_START" ? <><section aria-label="Passage progress" className={`rounded-2xl border border-[#d6e5d7] bg-[#f2f8f1] p-4 ${focusMode ? "mx-auto w-full max-w-2xl" : ""}`}><div className="mb-2 flex items-center justify-between text-sm text-[#426359]"><span className="font-semibold">Reading progress</span><span>Part {readingProgress.currentPart} of {readingProgress.totalParts}</span></div><Progress value={readingProgress.percentage} className="h-2 bg-[#d7e5d6] [&>div]:bg-[#609873]" /><p className="mt-2 text-xs leading-5 text-[#607a70]">This is only a place marker to help you navigate the passage. It is not a score.</p></section><article className={`rounded-3xl border border-[#dfe7df] bg-white px-6 py-8 shadow-sm sm:px-10 ${focusMode ? "border-0 bg-transparent shadow-none" : ""}`} style={{ fontSize: `${textScale}rem`, lineHeight: spacingValue[lineHeight] }}><div className="mx-auto max-w-2xl font-serif text-[#254b43]"><p className="tracking-[0.01em]">{currentSection}</p></div></article><section aria-label="Reading bookmark" className="flex flex-col gap-2 rounded-2xl border border-dashed border-[#cfe0d2] bg-white/70 p-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-[#49695d]">{bookmarkIndex === null ? "Save this part if you would like to come back to it." : `Your saved place is part ${getChildReadingProgress(sections.length, bookmarkIndex).currentPart}.`}</p><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" className="gap-2 bg-white" onClick={() => { setBookmarkIndex(readingProgress.index); window.localStorage.setItem(`reader-leader-demo-bookmark:${token}`, String(readingProgress.index)); }}><Bookmark className="h-4 w-4" /> Save my place</Button>{bookmarkIndex !== null && bookmarkIndex !== readingProgress.index ? <Button type="button" variant="outline" size="sm" className="gap-2 bg-white" onClick={() => setSectionIndex(getChildReadingProgress(sections.length, bookmarkIndex).index)}><BookmarkCheck className="h-4 w-4" /> Return to saved part</Button> : null}{bookmarkIndex !== null ? <Button type="button" variant="ghost" size="sm" className="text-[#5b746c]" onClick={() => { setBookmarkIndex(null); window.localStorage.removeItem(`reader-leader-demo-bookmark:${token}`); }}>Clear saved place</Button> : null}</div></section>{sections.length > 1 ? <nav className="flex gap-3" aria-label="Passage parts"><Button type="button" variant="outline" className="flex-1 gap-2 bg-white" disabled={readingProgress.index === 0} onClick={() => setSectionIndex(index => Math.max(0, index - 1))}><ArrowLeft className="h-4 w-4" /> Earlier part</Button><Button type="button" variant="outline" className="flex-1 gap-2 bg-white" disabled={readingProgress.index >= sections.length - 1} onClick={() => setSectionIndex(index => Math.min(sections.length - 1, index + 1))}>Next part <ArrowRight className="h-4 w-4" /></Button></nav> : null}</> : null}
